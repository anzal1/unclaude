        ...
        "skills": [
            {
                ...
                "steps_preview": [getattr(s, 'description', 'step')[:50] for s in skill.steps[:3]],
            }
...
async def create_skill(request: CreateSkillRequest):
    ...
    skill_path = Path.home() / ".unclaude" / "skills" / \
        f"{request.name.strip()}.yaml"

    if skill_path.exists():
        raise HTTPException(
            status_code=400, detail=f"Skill '{request.name}' already exists")
