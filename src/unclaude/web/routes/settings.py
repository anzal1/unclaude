...
async def get_settings():
    ...
    for provider_name, provider_info in PROVIDERS.items():
        ...
        settings["providers"][provider_name] = {
            ...
            "has_key": bool(provider_config.get("api_key") or credentials.get(provider_name) or (env_var and os.environ.get(env_var))),
            "env_var": env_var,
        }
...
async def remove_custom_model_endpoint(provider: str, model: str):
    ...
    if success:
        ...
        raise HTTPException(
            status_code=404, detail=f"Custom model '{model}' not found for {provider}")
...
async def delete_api_key(provider: str):
    ...
    raise HTTPException(
        status_code=404, detail=f"No API key found for {provider}")
